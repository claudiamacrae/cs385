...
int quick_select(int array[], size_t left, size_t right, size_t k) {
    ...
    if(s > k - 1) return quick_select(array, left, s - 1, k);
...
int main(int argc, char *argv[]) {
    ...
    if(k > num_values){
        string plural = (num_values > 1) ? " values." : " value.";
        cerr << "Error: Cannot find smallest element " << k << " with only " << num_values << plural << endl;
        return 1;
    }
