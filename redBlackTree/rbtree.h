...
public:
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) {
        ...
        if(find(key) != end()){         //duplicate key exists
            std::stringstream e;
            e << "Attempt to insert duplicate key '" << key << "'.";
            throw tree_exception(e.str());
        }
        while(x != NULL){
            y = x;
            if(key < x->key()){
...
private:
    ...
    void left_rotate(Node<K, V> *x) {
        ...
        if(x->parent == nullptr){
            root_ = y;
        }else if(x->parent->left == x){        //if x is the left child
            x->parent->left = y;
        }else{
            x->parent->right = y;               //must be right child
        }
        y->left = x;
        x->parent = y;
    }
    ...
    int diameter(Node<K, V> *node) const {
        ...
        if((lheight + rheight + 3) > ld){
            return lheight + rheight + 3;
        }
        return ld;
    }
